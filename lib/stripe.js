...
async function retrieveEvent(eventId) {
  const key = requireSecret();
  const response = await fetch(`${STRIPE_API}/events/${encodeURIComponent(eventId)}`, {
    headers: { Authorization: `Bearer ${key}` }
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data?.error?.message || `Stripe event lookup returned ${response.status}`);
  return data;
}

module.exports = { createCheckoutSession, retrieveEvent };
