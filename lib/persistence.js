...
async function loadState() {
  ...
  if (!result.rowCount) {
    ...
    const retry = await pool.query('SELECT version,state FROM titan_state WHERE id=$1');
    const row=retry.rows[0]; const state=row.state; state._version=Number(row.version); state._persistence={mode:'postgres',version:Number(row.version)}; return state;
  }
...
async function diagnostics() {
  if (!DATABASE_URL) return { configured: false, reachable: false, mode: 'local-json', error: 'DATABASE_URL is not configured' };
  try {
    const pool = await getPool();
    await pool.query('SELECT 1');
    await ensureSchema(pool);
    return { configured: true, reachable: true, mode: 'postgres', schemaReady: true };
  } catch (error) {
    return { configured: true, reachable: false, mode: 'postgres', schemaReady: false, error: error.message };
  }
}

function mode(){ return DATABASE_URL ? 'postgres' : 'local-json'; }
module.exports={loadState,saveState,mode,diagnostics};
